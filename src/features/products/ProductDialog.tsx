...
export default function ProductDialog({ isOpen, onClose, onSaved, initialProduct }: ProductDialogProps) {
  ...
  const [minStockAlert, setMinStockAlert] = useState(initialProduct?.min_stock_alert?.toString() || '5');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const product: Product = {
      ...
      code: initialProduct?.code || 'PRD-' + Math.floor(100 + Math.random() * 900),
      ...
      <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-md p-6">
        <div className="flex items-center justify-between pb-4 border-b border-slate-100">
          <h3 className="font-bold text-slate-900 flex items-center gap-2">
            ...
            <span>{initialProduct ? 'تعديل بيانا المنتج' : 'إضافة منتج جديد'}</span>
          </h3>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4 text-xs">
          <div>
            <label className="font-semibold text-slate-700 block mb-1">اسم المنتج *</label>
            <input
              type="text"
              required
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="مثال: شاشة 55 بوصة Smart"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="font-semibold text-slate-700 block mb-1">سعر الشراء (EGP)</label>
              <input
                type="number"
                step="0.01"
                required
            ...
            <div>
              <label className="font-semibold text-slate-700 block mb-1">سعر البيع (EGP)</label>
              <input
                type="number"
                step="0.01"
                required
                value={sellingPriceEGP}
                onChange={(e) => setSellingPriceEGP(e.target.value)}
                placeholder="0.00"
                className="w-full px-3 py-2 border border-slate-300 rounded-lg font-bold dir-ltr"
              />
            </div>
          ...
          <div>
            <label className="font-semibold text-slate-700 block mb-1">الحد الأدنى لتنبيه المخزون</label>
            <input
              type="number"
              value={minStockAlert}
