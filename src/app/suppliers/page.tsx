...
import { Plus, DollarSign, Search, Truck } from 'lucide-react';
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-50">
                      ...
                      <td className="p-3.5 font-bold dir-ltr">
                        <span className={netBalanceCents < 0 ? 'text-rose-600' : 'text-emerald-600'}>
                          {formatCurrency(Math.abs(netBalanceCents))}
                        </span>
                      </td>
                      <td className="p-3.5 text-center">
                        <button
                          onClick={() => { setPaymentParty(p); setIsPaymentOpen(true); }}
                          className="bg-rose-50 hover:bg-rose-100 text-rose-700 px-3 py-1 rounded border border-rose-200 font-bold inline-flex items-center gap-1"
                        >
