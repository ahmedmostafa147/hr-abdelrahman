    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <button onClick={() => { setSelectedParty(null); setIsDialogOpen(true); }} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-xs font-semibold flex items-center gap-2 shadow-xs">
            <Plus className="w-4 h-4" /> <span>إضافة عميل جديد</span>
          </button>
        </div>

        <PageGuide
          title="دليل شاشة العملاء"
          description="تستعرض هذه الشاشة كود وبيانات كل عميل مع إجمالي مديونيته اللحظية المستحقة لصالحك."
          points={[
            'المديونية تُحسب تلقائياً: تزداد فور إنشاء أي فاتورة بيع آجلة، وتقل عند تسجيل عملية تحصيل نقدية.',
            'زر (تحصيل نقدًا) يقوم بإنشاء قيد قبض محاسبي متزن: زيادة الخزينة وتقليل ديون العميل بالدفتر العام.',
          ]}
        />
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
          <div className="relative w-full sm:w-80">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="بحث باسم العميل، الكود، أو الهاتف..." className="w-full pl-3 pr-9 py-2 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
        </div>
                ...
                {filteredParties.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-50">
                      ...
                      <td className="p-3.5 font-bold dir-ltr"><span className={netBalanceCents > 0 ? 'text-amber-600' : 'text-emerald-600'}>{formatCurrency(netBalanceCents)}</span></td>
                      <td className="p-3.5 text-center">
                        <button onClick={() => { setPaymentParty(p); setIsPaymentOpen(true); }} className="bg-emerald-50 hover:bg-emerald-100 text-emerald-700 px-3 py-1 rounded border border-emerald-200 font-bold inline-flex items-center gap-1">
