...
import { Landmark, Wallet, Users, Package, Save, CheckCircle2 } from 'lucide-react';
...
export default function OpeningBalancesPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (cashCents > 0) {
      lines.push({ account_id: DEFAULT_ACCOUNT_IDS.CASH, debit_cents: cashCents, credit_cents: 0, description: 'رصيد الخزينة الافتتاحي' });
      totalDebit += cashCents;
    }
    if (bankCents > 0) {
      lines.push({ account_id: DEFAULT_ACCOUNT_IDS.BANK, debit_cents: bankCents, credit_cents: 0, description: 'رصيد البنك الافتتاحي' });
      totalDebit += bankCents;
    }
    ...
    if (netCapital > 0) {
      lines.push({ account_id: DEFAULT_ACCOUNT_IDS.CAPITAL, debit_cents: 0, credit_cents: netCapital, description: 'موازنة الأرصدة الافتتاحية برأس المال' });
    } else if (netCapital < 0) {
      lines.push({ account_id: DEFAULT_ACCOUNT_IDS.CAPITAL, debit_cents: Math.abs(netCapital), credit_cents: 0, description: 'موازنة الأرصدة الافتتاحية برأس المال' });
    }

    const entry: JournalEntry = {
      id: 'je_open_' + Date.now(),
      entry_number: 'JV-OPEN-001',
      entry_date: new Date().toISOString(),
      description: 'قيد إثبات الأرصدة الافتتاحية وأول المدة',
      status: 'POSTED',
    ...
    <DashboardLayout>
      <div className="space-y-6 max-w-4xl">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">تحديد أرصدة الخزينة، البنوك، مديونيات العملاء، واستحقاقات الموردين السابقة لإنشاء قيد البداية المحاسبي</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-slate-200 p-6 shadow-xs space-y-6 text-xs">
          ...
          <div className="space-y-3 border-b border-slate-100 pb-5">
            <h3 className="font-bold text-slate-900 flex items-center gap-2">
              ...
              <span>1. أثر النقدية والسيولة المتاحة (خزينة وبنك)</span>
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="font-semibold text-slate-700 block mb-1">رصيد الخزينة الرئيسية (EGP)</label>
                <input type="number" step="0.01" value={cashEGP} onChange={(e) => setCashEGP(e.target.value)} placeholder="0.00" className="w-full px-3 py-2 border border-slate-300 rounded-lg font-bold dir-ltr" />
              </div>
              <div>
                <label className="font-semibold text-slate-700 block mb-1">رصيد البنك الأهلي (EGP)</label>
                <input type="number" step="0.01" value={bankEGP} onChange={(e) => setBankEGP(e.target.value)} placeholder="0.00" className="w-full px-3 py-2 border border-slate-300 rounded-lg font-bold dir-ltr" />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 border-b border-slate-100 pb-5">
            <h3 className="font-bold text-slate-900 flex items-center gap-2">
              ...
              <span>2. أرصدة مديونيات العملاء واستحقاقات الموردين السابقة</span>
            </h3>
            {parties.length === 0 ? (
              <p className="text-slate-400">قم بإضافة العملاء والموردين أولاً من شاشة الأطراف لتحديد أرصدتهم الحالية.</p>
            ) : (
                ...
                {parties.map((p) => (
                  <div key={p.id} className="flex items-center justify-between p-2.5 bg-slate-50 border border-slate-200 rounded-lg">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500">{p.party_type === 'CUSTOMER' ? 'عميل' : p.party_type === 'SUPPLIER' ? 'مورد' : 'مزدوج'}</p>
                    </div>
                    <input
                      type="number"
                      step="0.01"
                      placeholder="0.00 EGP"
                      value={partyBalances[p.id] || ''}
                      onChange={(e) => handlePartyChange(p.id, parseFloat(e.target.value) || 0)}
                      className="w-36 px-3 py-1.5 bg-white border border-slate-300 rounded font-bold dir-ltr"
                    />
                  </div>
                ))}
              </div>
          ...
          <div className="flex justify-end pt-2">
            <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-lg font-bold text-xs flex items-center gap-2 shadow-md">
              ...
              <span>حفظ وترحيل قيد الأرصدة الافتتاحية</span>
            </button>
          </div>
        </form>
      </div>
